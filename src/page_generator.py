...
public_dir = "/home/mattls/workspace/github.com/mattls95/static-site-generator/docs"
...
def generate_pages_recursive(base_path, dir_path_content=content_dir, template_path=template_path, dest_dir_path=public_dir,):
    directories = os.listdir(dir_path_content)
    for dir in directories:
        if os.path.isfile(os.path.join(dir_path_content, dir)) and dir.endswith(".md"):
            generate_page(base_path,os.path.join(dir_path_content, dir), template_path, os.path.join(dest_dir_path, dir).replace(".md", ".html"))
        else:
            generate_pages_recursive(base_path, os.path.join(dir_path_content, dir), template_path, os.path.join(dest_dir_path, dir))
...
def generate_page(base_path, from_path, template_path, dest_path):
    ...
    markdown_template = markdown_template.replace('href="/', f'href="{base_path}')
    markdown_template = markdown_template.replace('src="/', f'src="{base_path}')
